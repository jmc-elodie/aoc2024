...
let parseProgram (str: string) : ProgramState =
    ...
    let parseRegister (line: string) =
        ...
        (m.Groups.[1].Value.Chars 0 |> registerFromChar, m.Groups[2].Value |> Int32.Parse)
...
let setReg (ps: ProgramState) (reg: Register) (value: int) : ProgramState =
    { ps with regs = setCopy (regToInt reg) value ps.regs }

let setRegAndAdvance (ps: ProgramState) (reg: Register) (value: int) : ProgramState =
    { ps with regs = setCopy (regToInt reg) value ps.regs; ip = ps.ip + 2 }
    
let getReg (ps: ProgramState) (reg: Register) : int =
    ps.regs[regToInt reg]

let getComboOp (ps: ProgramState) : int =
...
module PartTwo =
    ...
    let solve (ps: ProgramState) : int =
        let tryAValue (a: int) =
            let ps = setReg ps A a |> runUntilHalt 
            let out = ps.out |> List.rev |> List.toArray
            out = ps.ops
        [0..10_000_000] |> Seq.find tryAValue
